...
const Header = () => {
  ...
  const navLinks = [
    ...
    { path: "/youtube-downloader", label: "YouTube", icon: <Youtube className="h-4 w-4 text-red-500" />, comingSoon: true },
        ...
        <nav className="hidden md:flex items-center gap-2" aria-label="Main navigation">
          {navLinks.map((link) => (
            link.comingSoon ? (
              <span
                key={link.path}
                className="px-4 py-2 rounded-xl font-medium flex items-center gap-2 text-muted-foreground/50 cursor-not-allowed opacity-60"
              >
                {link.icon}
                {link.label}
                <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded-full uppercase font-bold">Soon</span>
              </span>
            ) : (
              <Link 
                key={link.path}
          ...
          <nav className="flex flex-col gap-2" aria-label="Mobile navigation">
            {navLinks.map((link) => (
              link.comingSoon ? (
                <span
                  key={link.path}
                  className="font-medium flex items-center gap-3 p-4 rounded-2xl text-muted-foreground/50 cursor-not-allowed opacity-60"
                >
                  {link.icon}
                  {link.label}
                  <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded-full uppercase font-bold ml-auto">Coming Soon</span>
                </span>
              ) : (
                <Link 
                  key={link.path}
