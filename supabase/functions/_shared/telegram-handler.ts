// Shared Telegram message handler used by both the webhook (instant)
// and the (legacy) long-polling cron.
...
const MINI_APP_KEYBOARD = {
  inline_keyboard: [[{ text: "🚀 Open Mini App", web_app: { url: "https://anytt.cc/tg" } }]],
};
...
async function tg(method: string, body: Record<string, unknown>, lovableKey: string, tgKey: string) {
  ...
  if (!res.ok) console.error(`Telegram ${method} failed [${res.status}]:`, JSON.stringify(data));
...
async function tgMultipart(
  ...
  if (!res.ok) console.error(`Telegram ${method} (multipart) failed [${res.status}]:`, JSON.stringify(data));
  return { ok: res.ok, data };
}
...
async function downloadBytes(url: string, maxBytes: number): Promise<{ bytes: Uint8Array; mime: string } | null> {
  try {
    ...
    if (lenHeader && parseInt(lenHeader) > maxBytes) return null;
...
export async function processMessage(
  ...
  for (let i = 0; i < detectedList.length; i++) {
    ...
    // Send "typing" action instead of a separate message — much faster UX
    tg("sendChatAction", { chat_id, action: "upload_video" }, lovableKey, tgKey).catch(() => {});
